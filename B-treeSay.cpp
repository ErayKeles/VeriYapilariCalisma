...
void say(node* tree, int& nodeCount, int& leafCount) {
    if (tree != NULL) {
        nodeCount++;
        if (tree->leaf) {
            leafCount++;
        ...
        for (int i = 0; i <= tree->count; i++) {
            say(tree->child[i], nodeCount, leafCount);
        }
    }
}
...
int main() {
    ...
    say(btree, nodeCount, leafCount);

    cout << "Node Sayýsý: " << nodeCount << endl;
    cout << "Yaprak Sayýsý: " << leafCount << endl;
    return 0;
}
